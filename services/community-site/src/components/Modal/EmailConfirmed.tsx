...
const EmailConfirmed = ({ onSuccess }: { onSuccess: () => void }) => {
  return (
    <div>
      ...
      <Text label="You can now log in with your credentials." variant="body2" color="textSecondary" style={{ marginBottom: '5%' }} />
      <Button label="OK" color="secondary" variant="contained" onClick={() => onSuccess()} fullWidth className="marginButton" />
    </div>
  )
}
