...
function useRedeem() {
  const parseClaim = (claimData: ClaimResponse): Claim => ({
    ...
    numberOfTokens: BigNumber.from(
      ethers.utils.parseUnits(claimData.numberOfTokens, 18)
    ),
  ...
  const formatClaimsForTable = (
    ...
    const transformedClaims = claimsIncoming.map((claim: ClaimResponse) => {
      return {
        ...
        numberOfTokens: BigNumber.from(
          ethers.utils.parseUnits(claim.numberOfTokens, 18)
        ),
    ...
    transformedClaims.sort(
      (claimA, claimB) =>
        claimA.createdAt.getTime() - claimB.createdAt.getTime()
    );
