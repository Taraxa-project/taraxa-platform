...
export default class PostgresConnector {
  ...
  public async getDelegationsFromDatabase(): Promise<
    { user: string; address: string; value: string }[]
  > {
    ...
    try {
      ...
      console.log(`Read ${data.length} delegation records`);
      return data;
    } catch (error) {
      console.error(error);
  ...
  public async getUserEmail(
    address: string,
  ): Promise<{ user: string; address: string; email: string }> {
    const client = new Client({
      host: this.host,
      port: this.port,
      database: this.database,
